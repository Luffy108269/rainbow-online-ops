...
> Luffy พิมพ์ภาษาคนธรรมดา (เปลี่ยนจากคำสั่งแบบเดิม 24 ก.ค. 69) — Claude แปลงเป็นพิธีกรรมเองอัตโนมัติ:

- **"ฉันถึงที่ทำงานแล้ว" / "ฉันกลับมาแล้ว" (ถึงบ้าน) = เริ่มงาน** → `git pull` + `clasp pull` ใน apps-script/ + อ่าน tracker แล้วสรุปว่าค้างตรงไหน ก่อนเริ่มทุกครั้ง
- **"ฉันกำลังกลับบ้าน" / "ฉันจะนอนแล้ว" = เลิกงาน** → อัปเดต tracker + commit + push + ยืนยันว่า working tree สะอาด (ถ้ามีของค้างยังไม่ push ให้รีบเก็บให้จบก่อนบอกลา)
...
### ไฟล์ทำงานข้ามเครื่อง (บทเรียน 24 ก.ค. 69 — Luffy ไปถึงที่ทำงานแล้วไม่มีไฟล์)
- **ไฟล์ deliverable ทุกตัวที่ Luffy ต้องใช้ต่อ (Excel/CSV/รายงาน) ต้องอัพขึ้น Google Drive โฟลเดอร์ "Project : Rainbow online ops" ทันทีที่สร้างเสร็จ** — ห้ามเก็บไว้ในเครื่องเดียว (Luffy ใช้ 3 เครื่อง ไฟล์ local เครื่องอื่นมองไม่เห็น)
- ห้ามเอาไฟล์ข้อมูลธุรกิจเข้า repo นี้เด็ดขาด (repo เป็น public) — Drive เท่านั้น
- วิธีอัพที่ใช้ได้จริง: Drive API multipart + token จาก `.clasprc.json` (connector MCP ส่งไฟล์ใหญ่ไม่ไหว) · จดชื่อไฟล์+ลิงก์ลง tracker ด้วยทุกครั้ง
